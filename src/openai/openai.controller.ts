...
export class OpenaiController {
  ...
  generateToken() {
    ...
    const srcretKey = this.configService.getOrThrow('app.secret_key', {
      infer: true,
    });
   ...
   */
  @Post(
    'send-message/:system_prompt_code/:user_prompt_code/:model_code/:session_id',
  )
  @ApiBearerAuth()
  @ApiBody({ type: CreateTemplateComplationChatDto })
  @ApiParam({
    name: 'system_prompt_code',
    description: 'ID of the code item to retrieve',
    example: 'four_pillars_of_destiny',
  })
  @ApiParam({
    name: 'user_prompt_code',
    description: 'ID of the code item to retrieve',
    example: 'fourPillarsOfDestiny',
  })
  @ApiParam({
    name: 'model_code',
    description: 'ID of the code item to retrieve',
  ...
  @ApiParam({
    name: 'session_id',
    description: 'ID of the code item to retrieve',
    example: 'fe10c0ec-8480-45e6-9f5b-f44cebfdabfb',
  })
  ...
  async sendMessage(
    ...
    const modelCodeItem = await this.codeItemService.findOneByCodeAndKey(
      'openai-model',
      modelCode,
    );
    const systemPromptCodeItem = await this.codeItemService.findOneByCodeAndKey(
      'system-prompt-template',
      systemPromptCode,
    );
    const userPromptCodeItem = await this.codeItemService.findOneByCodeAndKey(
  ...
  @ApiParam({
    name: 'uuid',
    description: 'The UUID of the ChatCompletion entity',
  }) // 경로 파라미터 설명
  @ApiResponse({
    status: 200,
    description: 'Successfully retrieved the chat completion.',
    type: ChatCompletion,
  }) // 성공 시 응답 설명
  @ApiResponse({ status: 404, description: 'ChatCompletion not found.' }) // 실패 시 응답 설명
  async findOne(@Param('uuid') uuid: string): Promise<ChatCompletion> {
  ...
  @ApiQuery({
    name: 'system_prompt_code_item_key',
    required: false,
    description: 'Optional system prompt code filter',
  })
  @ApiQuery({
    name: 'user_prompt_code_item_key',
    required: false,
    description: 'Optional system prompt code filter',
  })
  @ApiQuery({
    ...
    const where: FindOptionsWhere<OpenaiChatSession> = {
      completions: {
        ...(systemPromptCodeItemKey && {
          systemPromptCodeItem: await this.codeItemService.findOneByCodeAndKey(
            'system-prompt-template',
            systemPromptCodeItemKey,
          ),
        }),
        ...(userPromptCodeItemKey && {
          userPromptCodeItem: await this.codeItemService.findOneByCodeAndKey(
            'user-prompt-template',
            userPromptCodeItemKey,
          ),
        }),
