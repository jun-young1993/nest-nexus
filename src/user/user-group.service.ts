...
export class UserGroupService {
  ...
  async update(id: string, updateUserGroupDto: UpdateUserGroupDto): Promise<UserGroup> {
    ...
    if (updateUserGroupDto.name && updateUserGroupDto.name !== userGroup.name) {
      ...
      if (existingGroup) {
        throw new BadRequestException(`Group with name '${updateUserGroupDto.name}' already exists`);
      }
    }
  ...
  async remove(id: string): Promise<void> {
    ...
    if (userGroup.users && userGroup.users.length > 0) {
      throw new BadRequestException('Cannot delete group with active members. Remove all members first.');
    }
   ...
   * 사용자를 그룹에 추가
   */
  async addUsersToGroup(addUserToGroupDto: AddUserToGroupDto): Promise<UserGroup> {
    ...
    if (newUsers.length === 0) {
      throw new BadRequestException('All specified users are already members of this group');
    }
   ...
   * 사용자를 그룹에서 제거
   */
  async removeUsersFromGroup(removeUserFromGroupDto: RemoveUserFromGroupDto): Promise<UserGroup> {
    ...
    if (validUserIds.length === 0) {
      throw new BadRequestException('None of the specified users are members of this group');
    }

    // 사용자 제거
    userGroup.users = userGroup.users.filter(user => !validUserIds.includes(user.id));
    return await this.userGroupRepository.save(userGroup);
  }
