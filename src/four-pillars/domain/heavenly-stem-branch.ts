import {
  EarthlyBranche,
  EarthlyBranches,
  HeavenlyEarthlyInterface,
  HeavenlyStem,
  HeavenlyStems,
  TenGod,
  TenGods,
} from '../interfaces/heavenly-earthly.interface';
...
export default class HeavenlyStemBranch {
  ...
  constructor(solarLunar: SolarLunarInterface, hour: number, minute: number) {
  ...
  public getTenGod(heavenlyEarthly: HeavenlyStem | EarthlyBranche): TenGod {
    ...
    // 오행이 동일하고 음양도 같으니 비견
    if (
      heavenlyEarthly.element === dayHeavenly.element &&
      heavenlyEarthly.yinyang === dayHeavenly.yinyang
    ) {
      return {
        ko: '비견',
        hanja: '比肩',
    ...
    if (
      this.isGeneratingRelation(dayHeavenly.element, heavenlyEarthly.element) &&
      heavenlyEarthly.yinyang === dayHeavenly.yinyang
    ) {
      return {
        ko: '식신',
    ...
    if (
      this.isControllingRelation(
        dayHeavenly.element,
        heavenlyEarthly.element,
      ) &&
      heavenlyEarthly.yinyang === dayHeavenly.yinyang
    ) {
      return {
    ...
    if (
      this.isGeneratingRelation(heavenlyEarthly.element, dayHeavenly.element) &&
      heavenlyEarthly.yinyang === dayHeavenly.yinyang
    ) {
      return {
        ko: '편인',
  ...
  // 천간과 지지를 분리하여 객체 반환
  private getHeavenlyAndEarthly(
    heavenlyAndEarthly: string,
  ): HeavenlyEarthlyInterface {
  ...
  private getHourHeavenlyAndEarthly() {
    ...
    const timePeriod = timeRanges.find((range) => {
      ...
      if (
        this.hour > startHour ||
        (this.hour === startHour && this.minute >= startMinute)
      ) {
        if (
          this.hour < endHour ||
          (this.hour === endHour && this.minute < endMinute)
        ) {
          return true;
        }
      }
