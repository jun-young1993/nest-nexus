...
export class TasksService {
  ...
  async createAlieHotProductPromotionOpenAi() {
    if (true || this.configService.get('app.is_dev', { infer: true })) {
      this.logger.info(
        '[CREATE ALIE HOT PRODUCT PROMOTION][SKIP CREATE ALIE HOT PRODUCT PROMOTION IN DEVELOPMENT]',
      );
      return false;
    }
  ...
  async getAlieHotProductPromotion(
    ...
    try {
      ...
      if (categoryResponse?.aliexpress_affiliate_category_get_response) {
        const alieExpressAffiliateCategoryGetResponse =
          categoryResponse.aliexpress_affiliate_category_get_response;
        if (alieExpressAffiliateCategoryGetResponse?.resp_result) {
          const respResult =
            alieExpressAffiliateCategoryGetResponse?.resp_result;
          if (respResult?.resp_code && respResult?.resp_msg) {
            if (respResult?.resp_code === 200) {
              ...
              ] = respResult.result.categories.category;
              ...
              if (Array.isArray(categories)) {
                ...
                for (const category of categories) {
                  ...
                  const hotProductResponse =
                    await this.alieAffiliateService.queryHotProducts(
                      category.category_id,
                    );

                  if (
                    hotProductResponse?.aliexpress_affiliate_hotproduct_query_response
                  ) {
                    const hotProductResult =
                      hotProductResponse
                    ...
                    if (hotProductResult.resp_code === 200) {
                        ...
                        for (const product of products) {
                          ...
                          if (githubContentResult === null) {
                            ...
                            let userContent = `
                                                           ...
                                                           `;
                            this.logger.info(
                              '[ALIE HOT PRODUCT PROMOTION]\r\n' + userContent,
                            );
                            const product_small_images =
                              ...
                              if (product_small_images?.string) {
                                ...
                                if (Array.isArray(urls)) {
                                  for (const smallUrl of urls) {
                                    ...
                                    smallUrlLinks.push(
                                      `[![${productImageUrl}](${smallUrl})](${promotionLink})`,
                                    );
                                  }
                                }
                              }
                            ...
                            const resultContent =
                              await promptFunction(userContent);
                            const uploadContnet =
                              mainLinkImage +
                              '\r\n' +
                              hereLink +
                            ...
                            const githubCreateContent =
                              await this.githubContentService.createContent(
                                githubAlieRepository,
                                blogPath,
                                uploadContnet,
                              );
                            this.logger.info(
                              '[ALIE HOT PRODUCT PROMOTION][GITHUB CREATE CONTENT]\r\n' +
                      ...
                      throw new Error(
                        '[The "products" value is not of array type.]',
                      );
                    ...
                    throw new Error(
                      '[It it not a 405 or 200 success value] :' +
                        hotProductResult.resp_code +
                        ' ' +
                        hotProductResult.resp_msg,
                    );
                  }
                  throw new Error(
                    '[The Key value "aliexpress_affiliate_hotproduct_query_response" cannot be found]',
                  );
                }
            ...
            throw new Error(
              '[It it not a 200 success value] :' + respResult.resp_msg,
            );
          }
          throw new Error(
            '[The Key value "resp_code" OR "resp_msg" cannot be found]',
          );
        }
        throw new Error(
          '[The Key value "aliexpress_affiliate_category_get_resp_result" cannot be found]',
        );
      }
      throw new Error(
        '[The Key value "aliexpress_affiliate_category_get_response" cannot be found]',
      );
    } catch (error) {
  ...
  async createAlieHotProductPromotionGemini() {
    ...
    if (true || this.configService.get('app.is_dev', { infer: true })) {
      this.logger.info(
        '[CREATE ALIE HOT PRODUCT PROMOTION GEMINI][SKIP CREATE ALIE HOT PRODUCT PROMOTION IN DEVELOPMENT]',
      );
      return false;
    }
