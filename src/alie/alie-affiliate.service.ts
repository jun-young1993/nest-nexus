...
export class AlieAffiliateService {
  ...
  async businessApi(method: string, args?: object) {
    ...
    const params = {
      access_token: this.configService.get('alie.access_token', {
        infer: true,
      }),
  ...
  async getCategories() {
    return await this.businessApi('aliexpress.affiliate.category.get');
  }
  ...
  async queryHotProducts(category_id: number): Promise<any> {
    return await this.businessApi('aliexpress.affiliate.hotproduct.query', {
